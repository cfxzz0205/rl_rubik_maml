...
if __name__ == "__main__":
    ...
    if value_targets_method == model.ValueTargetsMethod.ZeroGoalValue:
        adapt_lr=0.0001
        meta_lr=0.00002
        adapt_steps=1
        task_num = 5
        maml = l2l.algorithms.MAML(net, lr=adapt_lr).to(device)
        opt = optim.AdamW(maml.parameters(), meta_lr)
        print('meta-initialization start')
        for Iter in tqdm(range(1000)):
            meta_train_loss = 0.0
            for task in range(task_num):  # 5 tasks as a batch      
                learner = maml.clone(first_order=True)

                s_x_t, s_weights_t, s_y_policy_t, s_y_value_t = model.sample_batch(scramble_buf, net, device, config.train_batch_size, value_targets_method)
                q_x_t, q_weights_t, q_y_policy_t, q_y_value_t = model.sample_batch(scramble_buf, net, device, config.train_batch_size, value_targets_method)

                for _ in range(adapt_steps):
                    policy_out_t, value_out_t = learner(s_x_t.to(device),)
                    support_loss=lossfn(policy_out_t, value_out_t, s_weights_t, s_y_policy_t, s_y_value_t,config)
                    learner.adapt(support_loss)

                policy_out_t, value_out_t = learner(q_x_t.to(device),)
                query_loss=lossfn(policy_out_t, value_out_t, q_weights_t, q_y_policy_t, q_y_value_t,config)
                meta_train_loss += query_loss


            meta_train_loss = meta_train_loss / task_num
            opt.zero_grad()
